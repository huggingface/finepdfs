...
from blocks.corrupted_utils import check_is_corrupted_or_encrypted
import os
import numpy as np
...
class PDFScannedPredictor(BaseMediaExtractor):
    name = "📄 - PDFScannedPredictor"

    def __init__(self, path_to_model: str, exclusion_writer: JsonlWriter = None, exclude_failed: bool = True, num_pages_to_sample: int = 8, timeout=60):
        self.extractor = PDFFeatureExtractor(num_chunks=1, num_pages_to_sample=num_pages_to_sample)
        self.path_to_model = path_to_model
        self.num_pages_to_sample = num_pages_to_sample
        self._model = None
        super().__init__(timeout=timeout, exclusion_writer=exclusion_writer, exclude_failed=exclude_failed)
    ...
    def extract(self, media_bytes: bytes | None, document_metadata: dict):
        ...
        try:
            ...
            if check_is_corrupted_or_encrypted(pymupdf_doc):
                return "<no content>", {
                    "extraction_error": "Document is corrupted or encrypted"
                }

            features_raw = self.extractor.extract_features(pymupdf_doc)
